...
public class GuiNavigationContainer extends GuiContainer {
	...
	public void onResize() {
		...
		int y0 = spacing;
		// If parent is available, position relative to it; otherwise fall back to screen edges
		if(this.parent != null) {
			...
			// Screen-fixed navigation when no parent is provided
			if(position == Position.TOP) y0 = spacing;
			if(position == Position.BOTTOM) y0 = screen.height - spacing - buttonHeight;
		}
